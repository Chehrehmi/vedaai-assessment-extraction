...
export function DocumentViewer({
  ...
  return (
    <div className="flex flex-col h-full bg-[#1b1c1c] text-white select-none">
      {/* Top Toolbar */}
      <div className="px-4 py-3 bg-[#241916] border-b border-white/10 flex items-center justify-between gap-4 shrink-0 flex-wrap">
        ...
        <div className="flex items-center gap-1 bg-black/40 p-1 rounded-xl border border-white/10">
          ...
          >
            ...
            <span>Answer Sheet ({answerSheet.pageCount}p)</span>
          </button>
          <button
            type="button"
            onClick={() => {
          ...
          >
            ...
            <span>Question Paper ({questionPaper.pageCount}p)</span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 bg-black/40 px-2 py-1 rounded-xl border border-white/10 text-xs">
            ...
            <span className="font-mono px-2 font-bold text-white/90">
          ...
          {/* Zoom Controls */}
          <div className="hidden sm:flex items-center gap-1 bg-black/40 px-2 py-1 rounded-xl border border-white/10 text-xs">
            <button
              type="button"
            ...
            <button
              ...
              className="font-mono px-2 font-semibold hover:text-[#ffb59f] transition-colors"
              title="Reset Zoom"
            >
              {Math.round(zoom * 100)}%
            </button>
      ...
      {uniqueOtherPages.length > 0 && activeDocType === 'answer_sheet' && (
        <div className="bg-[#a63b17] px-4 py-2 text-xs flex items-center justify-between gap-2 shadow-xs shrink-0">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-[16px]">auto_stories</span>
            <span>This answer also spans across page(s): {uniqueOtherPages.join(', ')}</span>
          </div>
          <div className="flex items-center gap-1">
            {uniqueOtherPages.map((pg) => (
              <button
                ...
                className="px-2 py-0.5 rounded bg-white text-[#a63b17] font-bold text-[11px] hover:bg-white/90"
              >
                Go to P.{pg} →
              </button>
            ))}
      ...
      <div className="flex-1 overflow-auto p-4 sm:p-8 flex items-center justify-center relative bg-[#121212]">
        <div
          className="transition-transform duration-150 origin-center max-w-full"
          style={{ transform: `scale(${zoom})` }}
        >
          {/* Document Paper Container */}
          <div className="relative bg-white rounded-lg shadow-2xl overflow-hidden border border-white/10 max-w-[850px] min-w-[300px]">
